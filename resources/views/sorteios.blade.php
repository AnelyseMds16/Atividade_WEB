    ...
    <div class="py-6 text-center">
        ...
        @endif        
            <table class="table table-sm">
                <thead>
                  <tr>
                    <th style="text-align:left;width: 50px" scope="col"><img  src="{{ asset('img/logo.png') }}" alt="" width="90" height="90"></th>
                        ...
                        <tr>
                            ...
                            @if($item->User->id == Auth::id())
                                <td style="text-align:center">
                                    <a href="/Sorteio/editar/{{$item->id}}" class="btn btn-primary">Editar</a>
                                </td>
                                <td style="text-align:center">
                                    <a href="/Sorteio/apagar/{{$item->id}}" class="btn btn-danger">Apagar</a>
                                </td>
                                @if($item->sorteioRealizado == 0)
                                    @if($item->totalParticipantes > 2)
                                        <td style="text-align:center">
                                            <a href="/Sorteio/sortear/{{$item->id}}" class="btn btn-primary">Sortear</a>
                                        </td>
                                    @endif
                                @else 
                                    <td style="text-align:center">
                                        <a href="/Sorteio/deletarSorteio/{{$item->id}}" class="btn btn-primary">Desfazer Sorteio</a>
                                    </td>
                                @endif 
                            @endif
                            @if($item->sorteioRealizado == 0)
        ...
        <div class="card-footer">
            <a href="/Sorteio/novo" class="btn btn-primary btn-sm" role="button">Novo Sorteio</a>
